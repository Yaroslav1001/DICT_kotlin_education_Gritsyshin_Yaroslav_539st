...
//    Наприклад, для [34, 15, 88, 2], відповідь 2.
    ...
    val size = 6
    val numbers = Array(size) { Random.nextInt(100) } // Створення рандомного масиву з цілими числами від 0 до 99
    val minimumNumbers = numbers.min() // визначення найменшого значення методом "min()"
    println("Рандомний масив: ${numbers.joinToString()}")
    println("Найменше значення масиву - $minimumNumbers")

//    6. Задано масив цілих чисел, для якого потрібно знайти два найбільші числа.

    val massive = List(size) { Random.nextInt(100) } // Створення рандомного списку з цілими числами від 0 до 99
    val sortedMassive = massive.sortedDescending() // проводиться сортування чисел за спаданням
    println("Рандомний список: $massive")
    println("Найбільші два числа масиву - ${sortedMassive[1]}, ${sortedMassive[0]}")   // у виводі обираю два перших числа з відсортованого списку (бо вони найбільші)
...
//Як відповідь має бути [[1,2,3],[2,4,6],[3,6,9]].
    ...
    if (n != null && n > 0) { //Перевіряється, чи значення n не є null і чи воно більше за 0.
        ...
        for (i in 0 until n) { // Цикл, який проходиться по рядках таблиці множення.

            for (j in 0 until n) { // Цикл, який проходиться по стовпцях у кожному рядку таблиці множення.
        ...
        // Виведення таблиці множення (матриця)
        for (row in multiplicationTable) {
            println(row.joinToString())
        }

        // Формування рядка списку
        val resultTable = mutableListOf<List<Int>>()
        for (row in multiplicationTable) {
            resultTable.add(row.toList())
        }

        // Виведення результату у вигляді рядка списку
        println("\n$resultTable")
    }
}
